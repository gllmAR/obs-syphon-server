...
static bool sy_output_start(void *data)
{
    ...
    if (!started) {
        ...
        obs_log(LOG_INFO, "[syphon] Started capture: %s, %dx%d", output_data->srv.name, output_data->width,
                output_data->height);
    }
...
static void sy_output_raw_video(void *data, struct video_data *frame)
{
    ...
    gs_texture_t *tex = gs_texture_create(output_data->width, output_data->height, GS_BGRA, 1,
                                          (const uint8_t **) &frame->data[0], GS_DYNAMIC);
    ...
    if (tex) {
        ...
        if (frame_counter % 300 == 0) {
            obs_log(LOG_INFO, "[syphon] Output processed frame #%d (%dx%d)", frame_counter, output_data->width,
                    output_data->height);
        }
    } else {
...
static obs_properties_t *sy_output_get_properties(void *data)
{
    ...
    obs_properties_add_text(props, "syphon_server_name", obs_module_text("SyphonOutput.ServerName"), OBS_TEXT_DEFAULT);
