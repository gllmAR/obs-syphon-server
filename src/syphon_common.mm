...
static void main_output_raw_video(void *data, struct video_data *frame)
{
    ...
    if (frame_counter % 300 == 0) { // Log every 5 seconds at 60fps
        obs_log(LOG_INFO, "[syphon] RAW VIDEO: Frame #%d, %dx%d, linesize=%d", 
                frame_counter, width, height, frame->linesize[0]);
        ...
        obs_log(LOG_INFO, "[syphon] Frame content: %s", 
                hasContent ? "HAS CONTENT" : "APPEARS BLACK");
    }
...
static bool main_output_start(obs_output_t *output)
{
    ...
    if (started) {
        ...
        obs_log(LOG_INFO, "[syphon] Main output data capture started: %dx%d", 
...
void sy_server_init(struct sy_server *srv, const char *name)
{
    ...
    if (context) {
        ...
        NSDictionary *options = @{
            SyphonServerOptionIsPrivate: @NO
        };
        
        srv->server = [[SyphonOpenGLServer alloc] 
        ...
        if (srv->server) {
            ...
            obs_log(LOG_INFO, "[syphon] Server description: %s", 
                    [[serverDescription description] UTF8String]);
        } else {
...
void sy_server_publish_frame(struct sy_server *srv, gs_texture_t *tex, uint32_t width, uint32_t height)
{
    if (!srv || !tex || width == 0 || height == 0) {
        obs_log(LOG_WARNING, "[syphon] Invalid parameters for publish_frame: srv=%p, tex=%p, %dx%d", 
                srv, tex, width, height);
        return;
    }
    ...
    [srv->server publishFrameTexture:textureID 
    ...
    if (log_counter % 300 == 0) { // Log every 5 seconds at 60fps
        obs_log(LOG_INFO, "[syphon] Server '%s': Publishing %dx%d (texture ID %u)", 
                srv->name, width, height, textureID);
...
void sy_server_publish_raw_frame(struct sy_server *srv, uint8_t *data, uint32_t width, uint32_t height, uint32_t linesize)
{
    if (!srv || !data || width == 0 || height == 0) {
        obs_log(LOG_WARNING, "[syphon] Invalid parameters for publish_raw_frame: srv=%p, data=%p, %dx%d", 
                srv, data, width, height);
        return;
    }
